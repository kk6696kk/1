...
def evaluate(
    ...
    **kwargs,
    ...
    logger.info("Evaluating experiment/model: %s", experiment)

    # --- 添加新的加载逻辑分支 ---
    # 我们通过检查 'experiment' 参数是否指向一个 .pth 文件来决定加载方式
    if experiment.endswith(".pth"):
        # 这是我们的新流程：加载独立训练的特征提取器
        logger.info("Detected .pth file. Activating custom feature extractor loading workflow.")
        
        # 1. 检查权重文件是否存在
        feat_ext_ckpt_path = Path(experiment)
        if not feat_ext_ckpt_path.exists():
            raise FileNotFoundError(f"Provided .pth checkpoint not found: {feat_ext_ckpt_path}")

        # 2. 创建一个空的、但结构正确的模型
        #    这依赖于你已经在配置文件中设置了 model.name = 'mobilenetv3'
        #    并且已经创建了 maploc/models/mobilenetv3.py
        logger.info(f"Initializing a blank model with architecture: '{cfg.model.name}'")
        model = GenericModule(cfg)

        # 3. 加载并移植权重
        logger.info(f"Loading feature extractor weights from: {feat_ext_ckpt_path}")
        state_dict = torch.load(feat_ext_ckpt_path, map_location='cpu')
        
        prefixed_state_dict = {
            f"model.feature_extraction.{k}": v for k, v in state_dict.items()
        }

        incompatible_keys = model.load_state_dict(prefixed_state_dict, strict=False)
        
        # 4. 检查加载结果
        if incompatible_keys.unexpected_keys:
            logger.error("[FATAL] Mismatch between model architecture and checkpoint!")
            logger.error("  Unexpected keys found: %s", incompatible_keys.unexpected_keys)
            raise RuntimeError("Model architecture mismatch.")
        logger.info("Successfully transplanted weights into the feature extractor.")

    else:
        # 这是原始的流程：加载一个完整的 Pytorch Lightning 检查点
        logger.info("Loading from a standard Lightning checkpoint.")
